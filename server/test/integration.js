  ...
  describe('authenticated state', async () => {
    ...
    describe('calculate-test', async () => {
      it('allows user to store their gender', async () => {
        ...
        // gender err
        const result = await client.post('/api/calculate', {
          userGender: '',
        ...
        assert.equal(
          result.data.calcError,
          'You must select a gender to calculate the macros',
        );
      });
      ...
      it('allows user to store their age', async () => {
        ...
        // age err
        let result = await client.post('/api/calculate', {
          userGender: 'female',
          userAge: '',
          userHeight: getRandomInt(120, 201),
        ...
        assert.equal(
          result.data.calcError,
          'You must enter an age to calculate the macros',
        );

        result = await client.post('/api/calculate', {
          userGender: 'female',
          userAge: -50,
          userHeight: getRandomInt(120, 201),
        ...
        assert.equal(
          result.data.calcError,
          'Please enter a valid age to calculate the macros',
        );
      });
      ...
      it('allows user to store their height', async () => {
        ...
        assert.equal(
          result.data.calcError,
          'You must enter a height (cm) to calculate the macros',
        );
        ...
        assert.equal(
          result.data.calcError,
          'Please enter a valid height (cm) to calculate the macros',
        );
      });
      ...
      it('allows user to store their weight', async () => {
        ...
        assert.equal(
          result.data.calcError,
          'You must enter a weight (kg) to calculate the macros',
        );
        ...
        assert.equal(
          result.data.calcError,
          'Please enter a valid weight (kg) to calculate the macros',
        );
      });
      ...
      it('allows user to store their activity level', async () => {
        ...
        assert.equal(
          result.data.calcError,
          'You must select an activity level to calculate the macros',
        );
      });
      ...
      it('calculates daily recommendations based on personal user values for male', async () => {
        ...
        const caloriesPerDay =
          (10 * 125 + 6.25 * 171 - 5 * 25) * activityFactor + 5;
      ...
      it('calculates daily recommendations based on personal user values for female', async () => {
        ...
        // get the activity factor based on activity level
        const activityFactor = 1.725;
        // calculate macros
        const caloriesPerDay =
          (10 * 50 + 6.25 * 160 - 5 * 22) * activityFactor - 161;
        const expectedMacros = {
          prot: Math.round(caloriesPerDay * 0.35),
          carbs: Math.round(caloriesPerDay * 0.35),
          fats: Math.round(caloriesPerDay * 0.3),
    ...
    describe('submit-test', async () => {
      it('allows user to track the macros calculated', async () => {
        ...
        // submit macros
        const result2 = await client.post('/api/submit', result.data.macros);
      ...
      it('throws error if user tracks macros without calculating them first', async () => {
        ...
        const macros = { prot: null, carbs: null, fats: null };
        const resultNull = await client.post('/api/submit', { macros });
        assert.equal(
          resultNull.data.submitError,
          'You must calculate macros before submitting',
        );
        // submit without any data
        const resultNoData = await client.post('/api/submit', { macros: null });
        assert.equal(
          resultNoData.data.submitError,
          'You must calculate macros before submitting',
        );
      });
      ...
      it('throws error if a registered user tries to submit macros without logging in', async () => {
        ...
        // submit macros
        const macros = { prot: 100, carbs: 200, fats: 300 };
        const result = await client.post('/api/submit', { macros });
        assert.equal(
          result.data.submitError,
          'You must be logged in to do that',
        );
      });
